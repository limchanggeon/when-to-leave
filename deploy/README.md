...
저장소가 공개라 VM 에서 그냥 `git clone` 하면 된다. 아래는 비공개이던 시절에
쓰던 방법으로, 사설 저장소로 되돌릴 때를 위해 남겨둔다.

> **`VITE_` 키는 숨길 수 없다.** 빌드가 클라이언트 번들에 그대로 넣으므로
> 사이트를 연 사람은 누구나 본다. 히스토리에 남은 값도 같은 이유로 문제가
> 아니다 — 지우나 마나 번들에 있다. 지키는 방법은 **콘솔에서 도메인을 묶는
> 것**이고, 카카오·구글 둘 다 걸려 있는 것을 확인했다(2026-09-08).
> 저장소는 그날 공개로 돌렸다.
