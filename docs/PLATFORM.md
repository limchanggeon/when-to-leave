...
- **어두운 곳은 판 하나뿐이다.** 판에는 늘 답이 걸린다 — 아직 물어보지
  않았으면 무엇을 답해줄지가, 물어봤으면 출발 시각이.
- **화면에서 제일 큰 것은 언제나 시각이다.** 꼬리말("에 나가세요")까지
  같이 키우면 문장이 커질 뿐 답이 커지지 않아서, 숫자만 키운다.
- **상자를 늘어놓지 않는다.** 같은 라운드·같은 그림자 카드를 나열하면
  무엇이 중요한지 사라진다. 머리말과 선으로 구역을 나눈다.
- **색이 시끄러운 자리는 호박색 하나뿐이다.** 상태 칩도 파스텔 알약이
  아니라 얇은 테두리로 구분한다.
