...
ODsay 는 배차 간격만 주므로, **열차 구간의 실제 출발 시각**은
[공공데이터포털 TAGO 열차정보](https://www.data.go.kr/data/15098552/openapi.do)에서
받아 붙입니다(`TAGO_SERVICE_KEY`, 무료·자동승인).

이게 붙어야 역산 엔진이 제 몫을 합니다. 시각표가 있는 구간은 **이산 구간**이 되어
"이 편을 타려면 언제까지 도착해야 하는가" 를 계산하고 앞 구간으로 데드라인을
넘깁니다 — 이 앱의 알맹이입니다.

```
목표 도착 21:00
  → KTX 00063  19:19 → 20:55   (21시 전에 닿는 가장 늦은 편)
  → "19:19에 나가세요"  여유 9분 포함
```

주의할 점:

- 오퍼레이션 이름이 **대문자로 시작**합니다(`GetStrtpntAlocFndTrainInfo`).
  소문자로 부르면 `[12] 해당 오픈API 서비스가 없거나 폐기됨` 이 나와
  키 문제로 오해하기 쉽습니다.
- 역 코드는 도시별로 따로 받아야 해서 처음 한 번이 느립니다. 한 번 받아 캐시합니다(343곳).
- 같은 구간이 여러 경로에 겹치므로 한 번만 조회해 나눠 씁니다 — 일일 호출 한도가 있습니다.
...
**도보 구간에는 좌표도 정류장 이름도 없습니다.** 그대로 두면 이름이 "경유지" 가 되고
지도에는 탈것 구간의 두 점만 찍혀 엉뚱한 직선이 그어집니다. 걷기는 결국 앞 구간이
끝난 곳에서 다음 구간이 시작하는 곳까지이므로, 양 끝을 이웃 구간에서 물려받습니다.
...
(ODPT GTFS + OpenTripPlanner 자체 호스팅 등). 국내에서 ODsay 가 해주는 일을
일본에서는 우리가 해야 한다는 뜻입니다.

일본어 UI 는 있지만 **데이터는 국내 전용**입니다 — 설계 문서의 "언어와 지역은
...
| Android(네이티브) | 가능 | `AlarmClock.ACTION_SET_ALARM` 인텐트. `EXTRA_SKIP_UI` 로 확인 화면도 생략 |
...
`src/alarm/` 에 provider 로 갈라 뒀습니다. 지금은 캘린더만 동작하고,
네이티브 둘은 "이 환경에서는 못 쓴다" 를 정직하게 돌려줍니다.
앱으로 감쌀 때 각 파일의 `schedule()` 만 채우면 나머지 코드는 그대로입니다.
...
| 가입 | 관리자가 하나씩 승인한다(SES 프로덕션 액세스 대기 중) |
