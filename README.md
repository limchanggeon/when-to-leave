...
**키가 없으면** `/api/route` 가 `no-credentials` 를 내고 레지스트리가 목업으로 넘어갑니다.
이때 화면 상단에 MOCK 배너가 뜹니다 — 배너는 어댑터 등록 여부가 아니라
**실제로 쓰인 구간의 `origin`** 을 보고 뜨므로, 실제 데이터가 오면 사라집니다.

### 알아둘 한계

ODsay 는 **구간 소요시간만 주고 출발 시각표는 주지 않습니다.** 그래서 실제 데이터로
오는 구간은 전부 연속 구간이고, 역산 엔진의 데드라인 전파는 열차 시간표 소스가
붙어야 제 몫을 합니다. 지금 실제 데이터가 주는 것은 "진짜 소요시간"까지입니다.
(KTX/SRT 시간표는 별도 소스가 필요 — k-skill 의 ktx-booking 은 파이썬 스크립트라
그대로 가져다 쓸 수 없고, 공공데이터포털 TAGO 열차정보 등을 붙여야 합니다.)
